...
namespace Mission.Repositories.Repositories
{
    public class MissionThemeRepository(MissionDbContext missionDbContext) : IMissionThemeRepository
    {
        ...
        {
            var missiononthemeExist = await _missionDbContext.MissionTheme.FirstOrDefaultAsync(m => m.Id == missionTheme.Id && !m.IsDeleted);
            if(missiononthemeExist == null)
            {
                return false;
            }
            missiononthemeExist.ThemeName = missionTheme.ThemeName;
            missionTheme.Status = missionTheme.Status;
